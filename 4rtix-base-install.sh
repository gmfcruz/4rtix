...
sed -i "s/\-gpt/\- gpt\n\    -vm/" /etc/calamares/modules/partition.conf 
sed -i "s/\-gpt/\- gpt\n\    -vm/" /etc/calamares-offline/modules/partition.conf  
sed -i "s/\-gpt/\- gpt\n\    -vm/" /etc/calamares-online/modules/partition.conf 

(cd /mnt) &
(cd /mnt/boot) &

runuser -l artix -c "calamares-config-switcher" &&
...
cryptdevice=UUID=${cryptuuid}:lvm-system loglevel=3 quiet resume=UUID=${swapuuid} net.ifnames=0 lsm=lockdown,yama,apparmor,bpf\"/g" \
        -i /mnt/etc/default/grub
sed -s 's/^#GRUB_ENABLE_CRYPTODISK=y/GRUB_ENABLE_CRYPTODISK=y/g' -i /mnt/etc/default/grub
 
artix-chroot /mnt pacman -S --noconfirm dosfstools freetype2 fuse2 gptfdisk libisoburn mtools os-prober iw memtest86+ wpa_supplicant device-mapper-runit lvm2-runit cryptsetup-runit havged-runit cronie-runit gcr networkmanager networkmanager-runit networkmanager-openvpn network-manager-applet ntp-runit acpid-runit syslog-ng-runit artools lsof strace wget htop mc zip unrar p7zip unzip hdparm smartmontools hwinfo dmidecode whois rsync nmap inetutils net-tools ndisc6 apparmor-runit dbus-runit
