...
artix-chroot /mnt dd if=/dev/random of=/crypto_keyfile.bin bs=512 count=8 iflag=fullblock
artix-chroot /mnt cryptsetup luksAddKey /dev/sdX2 /crypto_keyfile.bin
artix-chroot /mnt chmod 000 /crypto_keyfile.bin

artix-chroot /mnt mkinitcpio -p linux

artix-chroot /mnt psswd

artix-chroot /mnt pacman -S --noconfimr grub efibootmgr dosfstools freetype2 fuse2 gptfdisk libisoburn mtools os-prober iw memtest86+ wpa_supplicant

artix-chroot /mnt sed -i "s/quiet/quiet resume=UUID=`blkid -s UUID -o value /dev/lvmSystem/volSwap`/g" /etc/default/grub

artix-chroot /mnt sed -i "s/GRUB_CMDLINE_LINUX=\"\"/GRUB_CMDLINE_LINUX=\"cryptdevice=UUID=`blkid -s UUID -o value /dev/sda2`:lvm-system\"/g" /etc/default/grub

artix-chroot /mnt grub-install --target=x86_64-efi --efi-directory=/boot/EFI --bootloader-id=artix --recheck /dev/sda
...
artix-chroot /mnt artix-chroot /mnt ln -sf /etc/runit/sv/dmeventd /etc/runit/runsvdir/default
...
artix-chroot /mnt artix-chroot /mnt ln -sf /etc/runit/sv/networkmanager /etc/runit/runsvdir/default
...
artix-chroot /mnt ln -sf /etc/runit/sv/ntpd /etc/runit/runsvdir/default
artix-chroot /mnt ln -sf /etc/runit/sv/acpid /etc/runit/runsvdir/default
artix-chroot /mnt ln -sf /etc/runit/sv/syslog-ng /etc/runit/runsvdir/default
