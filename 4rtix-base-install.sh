...
parted -s -a optimal /dev/vda mkpart "primary" "fat16" "0%" "512MiB"
parted -s /dev/vda set 1 esp on
parted -s /dev/vda align-check optimal 1
parted -s -a optimal /dev/vda mkpart "primary" "ext4" "512MiB" "100%"
...
cryptsetup luksOpen /dev/vda2 lvm-system
...
basestrap /mnt base base-devel mkinitcpio grub
...
artix-chroot /mnt grub-install --target=x86_64-efi --efi-directory=/boot/EFI --bootloader-id=artix --recheck /dev/vda
